...
def evaluer_cnf(formule,list_var):
    #on parcourt les listes de formules 
    for i  in range(len(formule)):
        #si la clause de formuler[i] en tenant compte de listevar est fausse alors la formule est fausse 
        if evaluer_clause(formule[i], list_var) == False :
            #print(formule[i], list_var, evaluer_clause(formule[i], list_var))
            return False 
        #si c'est none on retourne none 
        elif evaluer_clause(formule[i], list_var) is None:
            #print(formule[i], list_var, evaluer_clause(formule[i], list_var))
            return None 
    #print(formule[i], list_var, evaluer_clause(formule[i], list_var))
    #si on ne rencontre aucun de ces cas on retourne True 
    return True  
...
def determine_valuations(list_var):
    #si y a pas de valeur indefinie alors on retourne la meme liste
    if None not in list_var:
        return [list_var]
    #on créé une liste qui va contenir les listes de listvar
    list_val = [list_var] 
    #on parcourt la premiere liste de listeval tant qu'il y a un None a linterieur  
    while None in list_val[0]:
        #on créé une nouvelle liste de valuation  qui a chaque itération de la boucle,
        # va contenir les nouvelles combinaisons de valeurs.
        nvlvaluation =[]
        #on parcourt les sous liste de listval qui represente une combi de valeur en cours de traitement 
        for souslist in list_val :
            #on prend les positions des endroit où il y a un none 
            index= souslist.index(None)
            #on crée des liste où on va remplacer le none par true et par false dans l'autre combinaison 
            valuation_vrai= souslist[:index]+ [True] + souslist[index+1:]
            valuation_fausse = souslist[:index]+[False]+ souslist[index+1:]
            #on met à jour dans nvl valuation 
            nvlvaluation.append(valuation_vrai)
            nvlvaluation.append(valuation_fausse)
        #quand while terminer donc plu de none dans la listval on ajoute les combi a list val
        list_val = nvlvaluation
    #on met a jour list var et on retourne 
    list_var = list_val
    return list_var
...
def progress(list_var,list_chgmts):
    #on parcourt la boucle i= iteration, valeur cest la valeur 
    for i, valeur in enumerate(list_var):
        #si on rencontre un none
        if valeur is None:
            #on change sa valeur en true 
            list_var[i]= True
            #on update dans listchgmt avec sa pos et true 
            list_chgmts.append([i,True])
            #on retourne les liste mis à jour 
            return list_var,list_chgmts
    #sinon si on rencontre pas de none on retourne les listes telles qu'elles sont 
    l1 = list_var
    l2 = list_chgmts
    return l1, l2
...
def retour(list_var,list_chgmts):
    #verifie si la liste est vide alors on renvoie les memes trucs
    if len(list_chgmts)==0:
        return list_var, list_chgmts
    #parcourt la listchgmt tant qu'elle n'est pas vide en commencant pas le dernier chgmt appliqué
    while list_chgmts :
        #recup la position et val du dernier chgmt et la suppr avec pop
        position,_ = list_chgmts.pop()
        #si val de pos actuelle est false on met none pour remonter 
        if list_var[position] == False:
            list_var[position] = None
        #sinon si true alors on remonte pour changer et tester avec false 
        elif list_var[position]== True:
            list_var[position] = False
           
        #verifie si nvl modification peut etre faite en verifiant si plus petite que listvar et est true ou false 
        if position < len(list_var) and list_var[position]== True or list_var[position]== False:
            #si respecte ces conditions alors nveau chgmt en reglant val a false ds listvar
            list_var[position] = False
             #et ajoute cette modif a list chgmt
            list_chgmts.append([position, False])
            #si ce nveau chgmt est a applique on renvoie directement celui-ci 
            return list_var, list_chgmts
    #sinon on renvoie le changement du debut telles qu'elles sont  
    return list_var, list_chgmts
    '''
    renvoie :l1,l2 avec :
